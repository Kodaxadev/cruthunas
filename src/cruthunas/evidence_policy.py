...
ORIGINATING_EVIDENCE_CLASSES = frozenset(
    {
        "CLAIM_REGISTRATION",
        "DERIVATION",
        "PROOF",
...
COMPUTATIONAL_ENVIRONMENT_KEYS = (
    "interpreter",
    "dependencies",
    "operating_system",
    "locale",
...
CLASSES_REQUIRING_COMMANDS = frozenset(
    {"COMPUTATION", "REPRODUCTION", "FORMALIZATION", "RELEASE"}
)
CLASSES_REQUIRING_ARTIFACTS = frozenset(
    {"COMPUTATION", "REPRODUCTION", "FORMALIZATION", "RELEASE"}
)
CLASSES_REQUIRING_ENVIRONMENT = frozenset(
    {"COMPUTATION", "REPRODUCTION", "FORMALIZATION", "RELEASE"}
)
...
def _actor_identity(record: Mapping[str, Any], field: str) -> str | None:
    ...
    if not isinstance(actor, Mapping):
        return None
    return normalized_identity(actor.get("id"))
...
def evidence_contract_errors(
    ...
    *,
    ...
    if evidence_class in CLASSES_REQUIRING_COMMANDS:
    ...
    if evidence_class in CLASSES_REQUIRING_ARTIFACTS:
        ...
        if not isinstance(environment, Mapping) or not environment:
            ...
            for key in COMPUTATIONAL_ENVIRONMENT_KEYS:
                if key not in environment or not _has_value(environment.get(key)):
                    errors.append(
                        f"{evidence_class} environment requires non-empty {key}"
                    )
    ...
    if required_keys:
        if not isinstance(details, Mapping):
            errors.append(
                f"{evidence_class} evidence requires structured class-specific details"
            )
        else:
            for key in required_keys:
                if key not in details or not _has_value(details.get(key)):
                    errors.append(
                        f"{evidence_class} evidence details require non-empty {key}"
                    )

    creator = _actor_identity(record, "created_by")
    originators = {
        key
        for value in originator_ids
        if (key := normalized_identity(value)) is not None
    }
    ...
    if evidence_class in {"REPRODUCTION", "REVIEW_EXTERNAL"}:
        ...
        if isinstance(details, Mapping) and not isinstance(
            details.get("saw_original_work"), bool
        ):
            errors.append(
                f"{evidence_class} evidence details require boolean saw_original_work"
            )

    if evidence_class == "REPRODUCTION" and creator is not None:
        if creator in originators:
            errors.append(
                "REPRODUCTION evidence creator must be independent of the claim originator"
            )
    ...
    if evidence_class == "REVIEW_EXTERNAL":
        ...
        if reviewer_type not in {"human", "venue"} or reviewer_identity is None:
            errors.append(
                "REVIEW_EXTERNAL evidence requires a named human reviewer or documented venue"
            )
        elif reviewer_type == "human":
            if creator is not None and reviewer_identity == creator:
                errors.append(
                    "REVIEW_EXTERNAL reviewer must differ from the evidence creator"
                )
            if reviewer_identity in originators:
                errors.append(
                    "REVIEW_EXTERNAL reviewer must be independent of the claim originator"
                )

    return errors


def claim_originator_identities(
    root: Path,
    claim_id: str,
    ...
    for record in records.values():
        if (
            ...
            and record.get("class") in ORIGINATING_EVIDENCE_CLASSES
        ):
