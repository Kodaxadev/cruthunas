...
Punctuated historical IDs remain manual while the alias contract cannot represent them losslessly. Independence-language detection is a conservative, bounded grammar rather than a general English semantic parser. It evaluates each match in its containing sentence and clause, propagates shared modal, evidential, hedge, and negative scope through directly coordinated independence predicates, and distinguishes modified direct subjects from embedded references such as a plan or requirement for future work. Actor nouns require a completed action performed by that actor; artifact nouns require a completed check or result attributable to that artifact; process nouns require completion, success, or a direct result predicate of the process itself. Supported actor roles include verifiers, reviewers, auditors, validators, and referees.

Action phrases must use completed-action morphology outside modal, progressive, planned, interrogative, uncertain, or attributed-report scope. Postposed and adjectival hedges are also excluded, and `independently` must attach directly to the supported completed action rather than an earlier neighboring verb. Blank lines, headings, list items, table rows, and fenced-block markers terminate predicate attribution; a single newline within the same prose block remains a supported hard wrap. The detector also excludes explicit negation, evidence absence, failed or unsuccessful attempts, abandoned, cancelled, or refused work, and ordinary mathematical uses such as “independently of `q`”; unrelated requirement or hedge wording and a neighboring question do not suppress a completed assertion. Structured evidence checks and unstructured prose checks run together during partial migration. A prose finding requests provenance review and does not establish that independence occurred. Absence of a finding does not establish that prose or structured evidence is complete.
